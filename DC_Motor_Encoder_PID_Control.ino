...
void loop() {
  ...
  int target_position = 6000; 
  //target = 250*sin(prevT/1e6);

 
  // (2) tunnig of PID constants
  float k_pro = 20; //proportional constant
  float k_int = 0.0;//integral constant
  float k_der = 0.7; //derivative constant
  
 
  // (3) difference in time
  long cu_time = micros(); //current time
  float time_diff = ((float) (cu_time - time_diff))/( 1.0e6 ); //delta time 
  prev_time = cu_time;
